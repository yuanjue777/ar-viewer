...
## 方块战线·玩法现状
- 横屏。左侧 3×3 出兵格**按列定职业**：蓝法师/绿游侠/红战士；中间 **3×13** 横向战线（战斗区加长过4格，COLS=16），怪从右来；最多 3 英雄（**开局免费送1个**，之后 100/200 金，招募走卡片弹层）。
...
- 逻辑在 rpg.js：`openHireCards()` 三张职业卡 / `openAdvCards(h)` 两张支线卡 / `closeCards()` / `refreshHire()`（刷新 dock 招募按钮价格与禁用态，满3人隐藏）。
...
## 已否决的方案（别再提）
- **战斗区改 5×n**（2026-07 用户问过，我算完劝退、用户采纳）：横屏手机 stage 只有约 190px 高，3 行时每格约 49px，加到 5 行掉到约 33px，模型细节全糊——和"模型要更华丽"直接冲突；而且英雄只有3个只能站3行，上下两行没人拦，25波的血量曲线和新手期折扣要整个重测。**替代方案已采用**：保持 3 行，靠加大怪的纵向散开+加快贴靠做包围感。
