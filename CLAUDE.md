...
- 横屏。**战斗区 5 行**（`ROWS=5`），横向 `COLS=20`（HCOLS=3 出兵 + BCOLS=17 战线），怪从右来；最多 3 英雄（**开局免费送1个**，之后 100/200 金，招募走卡片弹层）。
  ...
  - **裸跑基准（3英雄，不买任何技能装备、不升金木）**：w1~w4 **掉0命**，w5 掉到2命，w6 归零。（历史：w1~w8 全0命 → 提耐久曲线 → 小怪分工变肉 → 小怪血再×5，一路走到这里。裸跑是「什么都不买」的下限，玩家正常买东西不会这么惨。）比 3 行时代的"前5波0掉命"还好。改完平衡用这个基准对比。
...
- 开局是布阵阶段，点右上角 ▶ 才开波。共 **25 波**；逢5(5/15/25)是精英波、逢10(10/20)是纯Boss关，最后一波总攻。
  ...
  - **⚠️ 耐久和伤害是分开的（2026-07 用户指定）**：上面那条曲线现在只管**攻击力**；**血量在第5波之后额外 `× 1.09^(w-5)`**（`spawnMob` 里的 `hMul`）。所以后期压力是打不动而不是被秒。实测每波总血/总攻：w1 756/19 · w5 8189/165 · w8 34940/679 · w10 29963/278(Boss关) · w15 262078/3440 · w20 248734/976(Boss关) · w25 **6411381**/36379。**这个量级是用户确认过的**（英雄成长很快），别自作主张调低。
- **怪物属性（MOBS，2026-07 重做）**：⚠️ 用户定死两条 —— **移速全部 1.3（和英雄统一），多样化绝对不许靠移速**；**小怪整体要肉**。所以四种小怪靠 射程/伤害/护甲/血量 分工：
  | key | 定位 | 血 | 攻 | 甲 | 抗 | 攻击距离 | 扣命 |
  |---|---|---|---|---|---|---|---|
  | `normal` | 步兵·均衡 | 350 | 9 | 2 | 0 | .62 | 1 |
  | `lancer` | **长矛兵·手长**（原 `fast`，已改名） | 300 | 11 | 0 | 0 | **1.5** | 1 |
  | `brute` | **狂徒·高伤脆皮** | 275 | **26** | 0 | 0 | .5 | 1 |
  | `tank` | **重甲兵·高甲高抗** | 1200 | 15 | **14** | **30%** | .85 | 2 |
  | `boss` | Boss | 700 | 30 | 10 | 30% | 1.05 | 3 |
  - ⚠️ **四种小怪的血是用户 2026-07 亲自拍板上调到 5 倍的**（70/60/55/240 → 350/300/275/1200），Boss 没跟着动。
  - ⚠️ **由此产生两个倒挂，用户还没拍板，别自作主张改**：① `tank` 底血 1200 > `boss` 底血 700；② Boss关反而比相邻普通波轻松（w8 总血 34940 对 w10 Boss 29963；w15 总血 262078 对 w20 Boss 248734）。要修就是把 `boss.hp` 也 ×5，或者抬 `BOSS_HP_MUL`。
