...
- 横屏。**战斗区 5 行**（`ROWS=5`），横向 `COLS=20`（HCOLS=3 出兵 + BCOLS=17 战线），怪从右来；最多 3 英雄（**开局免费送1个**，之后 100/200 金，招募走卡片弹层）。
  ...
  - 上下两行没人拦，所以**英雄纵向选敌范围放宽过**：游侠 ±2.3 格、战士 ±1.15 格（原 1.6/0.7）。⚠️ 5 行后掉命会变多，**难度曲线还没重新实测**，要调就动 `waveComp`/`spawnMob` 的折扣。
...
- 金币+木头双货币。四个商店**入口是地图上的建筑**（不在 dock 了）：技能=紫顶法师书塔 / 装备=红顶铁匠铺 / 金矿=土黄矿洞 / 伐木场=绿顶锯木厂（后两个初始1级，每秒产出，上限10级）。
  - 建筑**竖排摆在战场左侧**（不是下方了；挪走才腾出纵向空间放第4/5行），常量在 rpg3d.js：`SHOP_X/SHOP_Z0/SHOP_DZ/SHOP_R/SHOP_S`，模型在 `buildShop()`。
  - **名牌画在建筑右侧**（`octx.textAlign='left'`）——竖排时画在上方会压到上一个建筑身上。技能/装备**不显示小字**（用户明确要求去掉），只有金矿/伐木场标 Lv 和产量。
...
- **怪物AI**：出生点在行附近**大幅随机散开**（`spawnMob` 里 ±0.65 格，移动时的纵向夹取也放宽到 ±0.32 格，否则一动就被拉回三条直线）；仇恨内纵向贴靠 `VEER_SPD=1.05` 格/秒——这三个数一起做出"一片压上来包围"的观感，沿自己路线向左走；仇恨半径 3.2；**攻击距离按种类**（atkR：快速0.42/普通0.62/坦克0.85/Boss1.05，**都<战士射程1.15**保证近战能还手）；仇恨内只以 0.7格/秒**轻微纵向贴靠**，一进攻击范围就停下开打（不会跑到英雄格子上）；**不排队**，各走各的，只在几乎重叠时纵向轻推 0.5格/秒。
