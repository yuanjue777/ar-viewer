...
- 横屏。**战斗区 5 行**（`ROWS=5`），横向 `COLS=20`（HCOLS=3 出兵 + BCOLS=17 战线），怪从右来；最多 3 英雄（**开局免费送1个**，之后 100/200 金，招募走卡片弹层）。
  ...
  - **裸跑基准（3英雄，不买任何技能装备、不升金木）**：w1~w5 **掉0命**，w6 归零（1敏=0.1攻速之后英雄 DPS 掉了约 1/3，从原来的「w6 剩4命」滑到这里；sim 有随机性 ±1 波正常）。裸跑是「什么都不买」的下限，玩家正常买东西不会这么惨。比 3 行时代的"前5波0掉命"还好。改完平衡用这个基准对比。
...
- 金币+木头双货币。四个商店**入口是地图上的建筑**（不在 dock 了）：技能=紫顶法师书塔 / 装备=红顶铁匠铺 / 金矿=土黄矿洞 / 伐木场=绿顶锯木厂。
  ...
  - **平衡推导（改数值前先看这段，别乱试）**：升级和招人互相放大（P=W×L），只比边际收益/花费——
    ...
    | 伐木场 | 同金矿 `60×lv` | 同金矿 `250×现有人数` | **2** | ⚠️ 2026-07 用户要求**和金矿完全同步**（原来是 150×lv / 110×w 的「人海型」，两栋楼的养法差太多）|
    - 满配：两栋楼都是 5人×Lv10 = **50/s**，各总花 **5200金**。实战不可能两边都点满——这就是选择点。木头现在是唯一的 roll 货币，所以伐木场直接决定「能roll多少次」。
...
- 开局是布阵阶段，点右上角 ▶ 才开波。共 **25 波**；逢5(5/15/25)是精英波、逢10(10/20)是纯Boss关，最后一波总攻。
  ...
  - **⚠️ 耐久和伤害是分开的（2026-07 用户指定）**：上面那条曲线现在只管**攻击力**；**血量在第5波之后额外 `× 1.09^(w-5)`**（`spawnMob` 里的 `hMul`）。所以后期压力是打不动而不是被秒。实测每波总血/总攻：w1 227/13 · w5 2457/109 · w8 10458/446 · w10 29963/278(Boss关) · w15 132596/2740 · w20 248734/976(Boss关) · w25 **3575885**/30110。**w25 这个量级是用户确认过可以的**（英雄成长很快），别自作主张调低。
- **怪物属性（MOBS，2026-07 重做）**：⚠️ 用户定死两条 —— **移速全部 1.3（和英雄统一），多样化绝对不许靠移速**；**小怪整体要肉**。所以四种小怪靠 射程/伤害/护甲/血量 分工：
  ...
  | `lancer` | **长矛兵·手长**（原 `fast`，已改名） | 90 | 7 | 0 | 0 | **1.5** | 1 |
  | `brute` | **狂徒·高伤脆皮** | 80 | **17** | 0 | 0 | .5 | 1 |
  | `tank` | **重甲兵·高甲高抗** | 360 | 10 | **14** | **30%** | .85 | 2 |
  | `boss` | Boss | 700 | 30 | 10 | 30% | 1.05 | 3 |
  - ⚠️ **这四种的血试过 ×5**（350/300/275/1200），用户实测「打不过」当场回退。**别再提整体翻倍**——它会把前期一起拖死；真要加后期压力就调 `spawnMob` 里的 `hMul` 曲线（现为第5波后 `×1.09^(w-5)`），那条按波次渐进，只压后期。
  - 上表是回退后又按用户要求「攻击再降、耐久再提」调过的一版：攻击 ×0.65 左右、血 ×1.5（原 70/60/55/240 攻 9/11/26/15）。
  - ⚠️ **调这张表前先想清楚它影响谁**：`sim` 裸跑里英雄从头到尾 `alive=3`，**掉命全来自漏怪**（怪走到左边界扣 `lives`），不是英雄被打死。所以
    **降小怪攻击对裸跑基准几乎没影响，加血反而会让基准变差**；降攻击真正帮到的是「英雄扛得住」，要在买了装备技能的实战里才体现。
...
- **⚠️ 平A一律单体（2026-07 用户要求）**：`CLASSES` / `ADV` / `MINIONS` 里的 `splash` **全部是 0**，法师和地狱火原来自带的溅射已删。**只有学了「攻击溅射」技能才溅射**（走 `h.skills['攻击溅射']` → `cleaveAround`，和 `splash` 是两套东西）。加新职业/召唤物时别再顺手写 splash。
- **伤害统计表（2026-07 加）**：屏幕左上角常驻一张 3 行小表（英雄 / 输出 / 承伤），实现在 rpg3d.js `draw()` 里紧挨 Boss 血条那段的 2D 覆盖层。
  - 数据靠 rpg.js 的全局 **`SRC`**（当前伤害来源英雄）串起来：英雄循环开头 `SRC=h`、召唤物循环 `SRC=br.own`、弹道结算 `SRC=s.src`、AOE(storms/quakes/hails) 用创建时存的 `own`；用完一律置回 `null`。
  - `damage()` 累加 `SRC.dmgOut`（不算溢出伤害），`hitUnit()` 累加 `h.dmgTaken`；**召唤物挨的打记在 `br.own` 头上**（用户要求召唤物承伤也算英雄的）。
  - ⚠️ **`SRC` 顺带承担法术强度**：`magDamage()` 里乘 `SRC.spellP`。所以新增任何"脱离英雄循环结算的魔法伤害"都必须先设 `SRC`，否则既不计入统计也吃不到法强。
- **仇恨修复（2026-07，用户报的"擦肩而过"）**：
  - 英雄选敌原来用硬 band 粗筛（战士 `|Δy|>1.15` 直接 continue），斜上/斜下一格的怪会被滤掉，英雄当没看见继续往前走 → 和 Boss 擦肩而过。**band 放宽到 战士1.6 / 游侠·法师3.2**，并且**0.9 格内的敌人优先锁定**（不再一味挑站得最靠左的那只）。
  - 召唤物原来是 `m.row!==br.row` **硬筛同一行**，所以会从斜下方的怪身边直接走过去。改成 1.6 格纵向 band + 距离判定，和英雄同一口径（1.1 格内优先）。
- **小怪弧形排开（2026-07）**：怪的分离从"只在几乎重叠时纵向轻推一次"改成**各向同性斥力**：期望间距 `SEP=r1+r2+.24`，越近推得越猛（`(SEP-d)/SEP*2.2*dt`），纵向再乘 1.35。因为所有怪都想停在离目标 `reach` 的位置，加上这层斥力就自然**围着肉盾摊成一道弧**，允许轻微重叠但数得清个数。
...
- **魔兽式属性系统**：主属性加攻击（战士主力/游侠主敏/法师主智）；**1力=8HP + 0.3生命回复/s**（`h.hpRegen`，2026-07 用户加的——在此之前英雄完全不回血；在 update 的英雄循环里和回蓝一起结算，和狂战士之血那种按失血比的回复相加）；敏捷/7加护甲、**1敏=0.3攻速点**（⚠️ 2026-07 从 1:1 一路调下来：先试 0.1，游侠 DPS 掉太多，定在 0.3。目的是让装备上的攻速真正值钱。改这一个系数 `h.agi*.3` 就能整体调游侠强度）；**1智=0.01回蓝/s + 0.001法术强度**（`h.spellP=1+int*.001`，在 `magDamage` 里对**所有**魔法伤害生效）；**攻速用魔兽/DotA正版公式：每秒攻击=(1+攻速/100)/BAT**（BAT1.4时+140攻速=每秒多A一下），攻速上限400；护甲减伤=甲×0.06/(1+甲×0.06)；魔抗固定百分比（英雄基础25%上限75%）。普攻=物理吃护甲，技能/毒/反伤=魔法吃魔抗。职业基础(攻击间隔bat/武器wep/甲)：法师1.6s/6/1(射程4.5)，游侠1.4s/8/2(射程6)，战士1.5s/15/**8**(射程1.15，护甲是用户2026-07从4提上来的)；成长/级：战士+2.7力，游侠+2.9敏，法师+3.2智（副属性各1~1.5）。怪物也有甲/抗：坦克6甲15%抗、Boss10甲30%抗。转职=属性×1.35+武器/HP/攻速倍率。
- **装备池（2026-07 用户整池重做，实现在 rpg.js 的 `EQUIPS` 表）**：**装备商店四档roll(`EQ_TIERS`)，每档固定出4件，⚠️ 只花木头**：低级200木(白60/绿30/蓝9/紫1)、中级400木(30/40/24/6)、高级800木(5/30/45/20)、**顶级1600木(不出白，绿5/蓝30/紫55/金10)**。⚠️ **顶级档是唯一能roll出金色的地方**（用户 2026-07 加的，推翻了原来「金色只从宝箱开」那条）；前三档仍然roll不到金色，宝箱那条路(`GOLD_DROP`)照旧。`rollEquip` 里金色要特判 `e.pool==='gold'`，别用 `!e.pool` 过滤。**和技能书同一个思路：roll 只是出货，真正穿上时按品质另付装备费**（`EQUIP_COST` **白50/绿120/蓝200/紫600/金1000 金币**，在 `applyItem` 里扣，背包格子和卡片层都直接标价，钱不够卡片标「金币不足」）。进背包点一下→选英雄穿（**装备栏2×3=6格**）。
...
**⚠️ 会话越长，同样的改动越贵（这条最容易被忽略）**
- 每一轮请求都会把**整段历史**重新送进去，所以「一个小改动花了 30% token」通常不是这轮做错了什么，
  而是历史里堆了十几张截图（每张约1500）、几段大 sed 输出、以及**开头注入的整份 CLAUDE.md**（现已 400+ 行）。
- 下面这些规矩只能压住**本轮新增**的量，压不住历史。真正的大杠杆是两条：
  ① **用户攒一批需求一起提**（同一份历史摊到多个需求上）；② **换话题时 `/clear`**。
- 所以：一轮里能一起做完的就别分两轮回；已经确认过的结论**别再复述第二遍**。

**核心认知：只有「进上下文的字」花 token。**
