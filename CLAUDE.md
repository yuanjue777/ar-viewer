...
- **智力系**：
  ...
  - 冰风暴（绿,主动CD10s,35蓝,AOE）：**暴风雪式**，目标区域(半径1.6+0.1lv)**每0.9秒降下1发**冰雹、共(3+lv)发，每发一小片AOE(半径0.7)，各20+智×lv魔法伤+减速40%+5%lv持续2s，同一敌人可被多发命中；整片范围圈+落地前0.5s显示下落特效(fx type 'fall')。实现用 hails 数组延迟落点。
