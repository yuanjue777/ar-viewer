...
- **智力系**：
  ...
  - 冰风暴（**紫**,主动CD10s,35蓝,AOE）：**暴风雪式**，**固定区域**(半径1.6+0.1lv)**每0.9秒对圈内全体结算一次**、共(3+lv)波，每波15+智×0.8lv魔法伤+减速40%+5%lv持续2s（参考魔兽暴风雪，同一敌人每波都吃）；整片范围圈+每波落地前0.5s撒下一片下落冰棱(fx type 'fall')。实现用 hails 数组存波次。
