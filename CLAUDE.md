...
- **魔兽式属性系统**：主属性加攻击（战士主力/游侠主敏/法师主智）；力量×8加HP；敏捷/7加护甲、×1.2%加攻速；护甲减伤=甲×0.06/(1+甲×0.06)；魔抗固定百分比（英雄基础25%上限75%）。普攻=物理吃护甲，技能/毒/反伤=魔法吃魔抗。职业基础(攻击间隔bat/武器wep/甲)：法师1.9s/6/1，游侠1.3s/8/2，战士1.7s/15/4；成长/级：战士+2.7力，游侠+2.9敏，法师+3.2智（副属性各1~1.5）。怪物也有甲/抗：坦克6甲15%抗、Boss10甲30%抗。转职=属性×1.35+武器/HP/攻速倍率。
- **装备系统**：商店roll随机装备（不固定）：部位5种（利剑+攻/胸甲+甲+HP/战靴+攻速/指环+全属性/披风+魔抗+HP），品质4档 白普通×1(55%)/绿精良×1.7(27%)/蓝稀有×2.6(13%)/紫史诗×4(5%)。单抽45金、四连160金，进背包拖到英雄穿（4装备位）。背包=技能书+装备混放一行。
- **力量系**：
...
- **智力系**：
  - 召唤熊德（蓝,主动CD22s）：召近战熊灵，会主动冲向本行最近怪贴脸砍（间距>0.8格冲锋1.8格/s，没怪撤回战线），怪会被熊挡住并攻击熊。HP=120+智×6lv，攻=8+智×0.7lv，持续12+2lv秒。
