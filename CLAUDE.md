...
- 横屏。**战斗区 5 行**（`ROWS=5`），横向 `COLS=20`（HCOLS=3 出兵 + BCOLS=17 战线），怪从右来；最多 3 英雄（**开局免费送1个**，之后 100/200 金，招募走卡片弹层）。
  ...
  - **裸跑基准（3英雄，不买任何技能装备、不升金木）**：w1~w5 **掉0命**，w6 掉到6命上下，w7 归零（sim 有随机性，±1波正常）。裸跑是「什么都不买」的下限，玩家正常买东西不会这么惨。比 3 行时代的"前5波0掉命"还好。改完平衡用这个基准对比。
...
- 开局是布阵阶段，点右上角 ▶ 才开波。共 **25 波**；逢5(5/15/25)是精英波、逢10(10/20)是纯Boss关，最后一波总攻。
  ...
  - **⚠️ 耐久和伤害是分开的（2026-07 用户指定）**：上面那条曲线现在只管**攻击力**；**血量在第5波之后额外 `× 1.09^(w-5)`**（`spawnMob` 里的 `hMul`）。所以后期压力是打不动而不是被秒。实测每波总血/总攻：w1 151/19 · w5 1638/165 · w8 6988/679 · w10 29963/278(Boss关) · w15 114266/3440 · w20 248734/976(Boss关) · w25 **3174176**/36379。**w25 这个量级是用户确认过可以的**（英雄成长很快），别自作主张调低。
- **怪物属性（MOBS，2026-07 重做）**：⚠️ 用户定死两条 —— **移速全部 1.3（和英雄统一），多样化绝对不许靠移速**；**小怪整体要肉**。所以四种小怪靠 射程/伤害/护甲/血量 分工：
  ...
  - ⚠️ **这四种的血试过再 ×5**（350/300/275/1200），用户实测「打不过」当场回退到上表。**别再提翻倍**——真要加压就走 `spawnMob` 里的 `hMul` 曲线，那条是按波次渐进的，不会把前期一起拖死。
