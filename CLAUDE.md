...
- 开局是布阵阶段，点 ▶ 才开波。共 **25 波**，逢5精英波、逢10纯Boss关、第25波总攻。
  ...
  - **BOSS关（逢10）**：**没有小怪只出1只Boss**，数值 `×BOSS_WAVE_MUL=3.0`、**血量再单独 `×BOSS_HP_MUL=3.0`**（只加血不加伤害）、体型×1.25。**BOSS总血条画在顶栏金币那一行**（`#uiBoss`，rpg.js 的 `updateBossBar()` 每帧刷 `scaleX`）——⚠️ 2026-07 用户要求从战场正上方挪进 header，因为会挡住战斗，别改回 rpg3d.js 的 2D 覆盖层。
...
- **伤害统计表**：屏幕**右上角**常驻 3 行（英雄/输出/承伤），实现在 rpg3d.js `draw()` 里（原 Boss 血条那段的位置，血条已挪走）。⚠️ 2026-07 用户要求从左上角挪到右上角，靠 `bx=W-bw-8` 右对齐，别改回 `bx=8`。
