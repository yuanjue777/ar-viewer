...
- **装备池（用户自定义，会持续加，实现在 rpg.html 的 EQUIPS 表）**：roll权重 白50/绿35/蓝15%（紫预留）。单抽45金、四连160金，进背包拖到英雄穿（**装备栏2×3=6格**）。背包=技能书+装备混放一行。
  - 白：新手剑+10攻；新手弓+15攻速；新手法杖 技能CD-5%。
  - 绿：狼牙锤+15力量；小圆盾+10护甲；毒牙之刃+15敏捷。
  - 蓝：雷鸣弓 攻击间隔-0.1且+20攻速；穷鬼盾 固定格挡普攻伤害20（先格挡后吃护甲）；法力指环 回蓝+2/s。
