...
- **转职分支系统**（ADV表2条/职业+SPECS专精表，实现在rpg.html）：Lv5花150金+250木转职，"转职▸"单按钮点开弹二选一支线(advPick状态)，各带专精技能。专精用金木升级(费=100lv金+60lv木,上限Lv5)。战士→狂战士(血怒:失血越多攻/攻速越高)/护卫(坚壁:+3甲×lv)；游侠→精灵游侠(迅捷:额外-0.2-0.01lv攻击间隔)/死亡射手(死神收割:敌死+0.5敏,上限20+10(lv-1))；法师→牧师(治愈祷言:主动30蓝CD8s回全体30+智×(0.6+0.4lv))/德鲁伊(自然之力:召唤物属性+20%×lv)。
...
- **智力系**：
  ...
  - 冰风暴（绿,主动CD10s,35蓝,AOE）：**暴风雪式**，目标区域(半径1.6+0.1lv)连续降下**(6+2lv)发**冰雹、间隔0.16s，每发一小片AOE(半径0.6)，各12+智×0.5lv魔法伤+减速40%+5%lv持续2s，同一敌人可被多发命中；整片范围圈+每发冰雹有下落特效(fx type 'fall')。实现用 hails 数组延迟落点。
