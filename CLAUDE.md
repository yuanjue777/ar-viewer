...
- **网页小游戏**（单文件零依赖，手机竖屏，先网页迭代后打包）：`www/td.html` 塔防；`www/rpg.html` 方块战线（横屏。3×3 出兵格按列定职业：蓝法师/绿游侠/红战士，3×n 横向战线怪从右来，最多3英雄(定价递增50/100/200)。金币+木头双货币；英雄升级、Lv5转职(150金+250木,分两条支线各带专精技能)；商店内容在常驻信息区内切换(不弹层)：技能书商店(力:狂战士之血/荆棘光环,敏:多重射击/沁毒射击,智:召唤熊德/火球术/冰风暴;80金4本进背包,roll半价40;拖到英雄学习,同名升级,4技能位)、装备、金矿、伐木场(初始1级,每秒产出)。怪物30秒一波自动来（共25波，强度=(1+0.12(w-1))×1.045^(w-1)，15波后再×1.06^(w-15)加速；w15≈5倍 w20≈10倍 w25≈20倍。单波数量封顶30只避免与下一波堆叠卡顿）、优先仇恨英雄会换行；开局布阵阶段，点右上角▶启动才开波；有伤害飘字、力敏智属性、AOE范围圈/单体弹道线特效。战士射程1.15格）。

## 方块战线·技能池（用户会持续加新技能，实现都在 www/rpg.html 的 SKB 表）
- 规则：技能书从商店买（指定系200金4本，全池roll半价100），进背包后**拖到英雄上学习**，同名书升级（上限Lv5），每英雄4技能位。拖到已占用的技能栏/装备栏=覆盖(会提示)；双击装备栏=脱下退回背包。背包最右"一键出售"清空背包换金(书30/装备白20绿40蓝80紫160)。蓝=稀有、绿=普通、紫=史诗。被动直接生效，主动自动释放（各自CD/蓝耗）。系数吃英雄力/敏/智属性。特效约定：**AOE显示范围圈，指定单体显示弹道线**。**买新技能书会清掉上次没用完的旧书**（不累积）。
