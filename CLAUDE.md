...
- **文件已拆分**（为省 token，别再合回单文件）：
  - `www/rpg.html`（约62行）= DOM 骨架：header(含 `#nextBtn`) / canvas / `#dock`(招募+4个 `.tile.trial`) / **`#bottom` = `#info`信息区 + `#inv`背包（同一行）** / `#cards`卡片层 / `#overlay`开始页
...
| 金矿/伐木场费用与上限 | `mineCost` / `INCOME_MAX` |
...
| **商店建筑模型 + 位置/点击半径**（竖排在左侧） | `function buildShop` / `const SHOPS=` / `SHOP_X` |
...
- 金币+木头双货币。四个商店**入口是地图上的建筑**（不在 dock 了）：技能=紫顶法师书塔 / 装备=红顶铁匠铺 / 金矿=土黄矿洞 / 伐木场=绿顶锯木厂（后两个初始1级，每秒产出，上限10级）。
  ...
  - **名牌画在建筑正下方**——2×2 时上方永远是另一个建筑，画上面会互相压。技能/装备**不显示小字**（用户明确要求去掉），只有金矿/伐木场标 Lv 和产量。
  - 点击流程：rpg.js 的 canvas pointerdown → `R3.shopAt(x,y)` 返回商店 key → `setShop(k)`。**商店内容仍然在常驻信息区内切换，不弹层**（`shopHTML()`）。**商店抬头只留标题**（"技能商店"/"装备商店"/"金矿 Lv4/10"），下面那几行说明文字用户明确要求去掉了，别再加回来。
...
- **英雄招募走卡片**：`HERO_COSTS=[0,100,200]`——**开局免费送1个**（begin() 里直接弹卡片让你选），之后 100/200 金。招募入口是 **dock 左边的「招募英雄」按钮**（`#hireBtn`，满3人自动 `display:none`），点空的出兵格也能唤起同一个卡片层。选了职业后自动放进该职业那一列的第一个空行（`freeRow(col)`）。
...
- **底栏布局（2026-07 改）**：信息区和背包**并排在同一行** `#bottom`（高 84px，含 safe-area），左边 `#info` 撑满剩余宽度，右边 `#inv` 固定 217px（左侧一小列=「背包」标题+出售按钮，右侧=**2 行 × 4 列 33px 物品格**，多了横向滚动）。这样比原来上下两行省了约 45px，全给战斗区。**商店按钮排成 2×2**（`.shopGrid`；装备商店只有 3 档，最后一档 `grid-column:1/-1` 占满整行），商店标题在最左侧竖条里。改底栏只动 rpg.css 的 `#bottom/#info/#inv/.shopGrid` 那几段 + `shopHTML()`。
