...
- 横屏。**战斗区 5 行**（`ROWS=5`），横向 `COLS=20`（HCOLS=3 出兵 + BCOLS=17 战线），怪从右来；最多 3 英雄（**开局免费送1个**，之后 100/200 金，招募走卡片弹层）。
  ...
  - **裸跑基准（3英雄，不买任何技能装备、不升金木）**：w1~w7 **掉0命**，w8 掉到2命，w9 归零（2026-07 大幅提耐久后从 w8 提前到 w7）。比 3 行时代的"前5波0掉命"还好。改完平衡用这个基准对比。
...
- 开局是布阵阶段，点右上角 ▶ 才开波。共 **25 波**；逢5(5/15/25)是精英波、逢10(10/20)是纯Boss关，最后一波总攻。
  ...
  - **⚠️ 耐久和伤害是分开的（2026-07 用户指定）**：上面那条曲线现在只管**攻击力**；**血量在第5波之后额外 `× 1.09^(w-5)`**（`spawnMob` 里的 `hMul`）。所以后期压力是打不动而不是被秒。实测每波总血/总攻：w5 1094/160 · w8 4151/451 · w10 29963/278(Boss关) · w15 99144/2720 · w20 248734/976(Boss关) · w25 2842555/30140。
- **怪物属性**（MOBS）：普通 40血/8攻，快速 26/6，坦克 150/14（6甲15%抗，扣2命），Boss 700/30（10甲30%抗，扣3命）。**移速全部 1.3**（和英雄统一）。
