...
- **网页小游戏**（单文件零依赖，手机竖屏，先网页迭代后打包）：`www/td.html` 塔防；`www/rpg.html` 方块战线（横屏。3×3 出兵格按列定职业：蓝法师/绿游侠/红战士，3×n 横向战线怪从右来，最多3英雄(定价递增50/100/200)。金币+木头双货币；英雄升级、Lv5转职(150金+250木,分两条支线各带专精技能)；商店内容在常驻信息区内切换(不弹层)：技能书商店(力:狂战士之血/荆棘光环,敏:多重射击/沁毒射击,智:召唤熊德/火球术/冰风暴;80金4本进背包,roll半价40;拖到英雄学习,同名升级,4技能位)、装备、金矿、伐木场(初始1级,每秒产出)。怪物**35秒**一波自动来（共25波，最后一波45小怪+3Boss总攻，强度=(1+0.12(w-1))×1.045^(w-1)，15波后再×1.06^(w-15)加速；w15≈5倍 w20≈10倍 w25≈20倍。单波数量封顶30只避免与下一波堆叠卡顿）。**怪物AI**：出生点在行附近随机散开(不固定格子中心)，沿自己路线向左走；仇恨半径3.2；**攻击距离按种类**(MOBS.atkR：快速0.42/普通0.62/坦克0.85/Boss1.05，都<战士射程1.15保证能还手)，仇恨内只以0.7格/秒**轻微纵向贴靠**，一进攻击范围就停下开打(不会跑到英雄格子上)；同伴挡路会排队。英雄选敌改用y距离(战士±0.7格/游侠±1.6格)；开局布阵阶段，点右上角▶启动才开波；有伤害飘字、力敏智属性、AOE范围圈/单体弹道线特效。战士射程1.15格）。
